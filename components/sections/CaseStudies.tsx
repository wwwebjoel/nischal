          ...
          >
                  ...
                  {mounted && (
                    <ResponsiveContainer width="100%" height={360}>
                      <AreaChart data={retirementData} margin={{ top: 10, right: 20, bottom: 0, left: 20 }}>
                        ...
                        <XAxis dataKey="age" tick={{ fill: "#8B9EC7", fontSize: 11 }} axisLine={false} tickLine={false} label={{ value: "Age", fill: "#8B9EC7", fontSize: 11, position: "insideBottom", offset: -2 }} />
                        <YAxis tick={{ fill: "#8B9EC7", fontSize: 11 }} axisLine={false} tickLine={false} tickFormatter={(v) => `$${v}K`} />
                  ...
                  {mounted && (
                    <ResponsiveContainer width="100%" height={340}>
                      <BarChart data={fundData} margin={{ top: 10, right: 20, bottom: 0, left: 10 }}>
                        ...
                        <Legend
                          formatter={(value) => <span style={{ color: "#8B9EC7", fontSize: 11 }}>{value}</span>}
                        />
                      ...
                      {insuranceData.map((item) => (
                        <div key={item.subject} className="space-y-1.5">
                          <div className="flex justify-between text-xs">
                            <span className="text-[#8B9EC7]">{item.subject}</span>
                            <span className="text-white font-semibold">{item.A}%</span>
                          </div>
                          <div className="h-2 bg-[#0F1E38] rounded-full overflow-hidden">
                            <motion.div
                              className="h-full rounded-full"
                              style={{ background: item.A >= 75 ? "#4CAF50" : item.A >= 60 ? "#C9A84C" : "#EF4444" }}
                              initial={{ width: 0 }}
                          ...
                          <p className="text-[#8B9EC7] text-[10px]">
                            {item.A >= 75 ? "Adequate" : item.A >= 60 ? "Review recommended" : "Underinsured — action required"}
                          </p>
                        </div>
                      ))}
                    </div>
            ...
            <div className="px-6 md:px-8 py-4 border-t border-white/[0.06] bg-[#0A1628]/50">
              <p className="text-[#8B9EC7] text-xs">
                * All data is anonymised and for illustrative purposes only. Real client data is kept strictly confidential in accordance with privacy obligations.
              </p>
            </div>
          </motion.div>
        </AnimatePresence>
